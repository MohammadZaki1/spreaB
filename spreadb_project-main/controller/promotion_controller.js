...
export const createPromotion = async (req, res) => {
  try {
    ...
    // Parse arrays from stringified data (if coming from form data)
    const categoriesArray = typeof categories === 'string' ? JSON.parse(categories) : categories;
    const locationsArray = typeof locations === 'string' ? JSON.parse(locations) : locations;
    const skillsArray = typeof skills === 'string' ? JSON.parse(skills) : skills;
    const otherLinksArray = typeof otherLinks === 'string' ? JSON.parse(otherLinks) : otherLinks;

    // Validate required fields
    if (!title || !budget || !duration) {
      return res.status(400).json({ 
        message: "Title, budget, and duration are required fields" 
      });
    }
    ...
    // Create promotion with a dummy brandOwnerId for now
    const dummyUserId = new mongoose.Types.ObjectId();
    
    const promotion = await Promotion.create({
      brandOwnerId: dummyUserId,
      title,
    ...
    return res.status(201).json({
      ...
      promotion: {
        ...
        otherLinks: promotion.otherLinks,
        categories: promotion.categories,
        locations: promotion.locations,
        skills: promotion.skills,
        budget: promotion.budget,
        budgetType: promotion.budgetType,
        duration: promotion.duration,
        requiredSticks: promotion.requiredSticks,
        images: promotion.images,
        status: promotion.status,
        applicationStatus: promotion.applicationStatus,
    ...
    // Clean up uploaded files if promotion creation fails
    if (req.files && req.files.length > 0) {
      req.files.forEach(file => {
        try {
